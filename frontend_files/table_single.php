...
$position = "<script>PositionIDValue()</script>";

$sql = "SELECT * FROM $table_applications WHERE position_id = %d";
$sql .= " ORDER BY surname, name";
$result = $wpdb->get_results($wpdb->prepare($sql, $position_id), ARRAY_A);
$count = $wpdb->num_rows;
if ($count < 0)
{
  exit;
